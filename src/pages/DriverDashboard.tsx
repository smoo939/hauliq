...
import { MapPin, Calendar, DollarSign, Truck, CheckCircle, Package, MessageCircle } from 'lucide-react';
...
function DriverLoads() {
  ...
  const { data: myLoadsForTracking } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('loads').select('*').eq('driver_id', user!.id).order('created_at', { ascending: false });
  ...
  const { data: availableLoads, isLoading: loadingAvailable } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('loads').select('*').eq('status', 'posted').order('created_at', { ascending: false });
      if (error) throw error;
      return data;
    },
    enabled: !!user,
  ...
  const { data: myLoads, isLoading: loadingMy } = useQuery({
    queryKey: ['driver-loads', user?.id],
    queryFn: async () => {
      const { data, error } = await supabase.from('loads').select('*').eq('driver_id', user!.id).order('created_at', { ascending: false });
      if (error) throw error;
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      ...
      <main className="px-4 py-4">
        <Routes>
          <Route index element={<DriverLoads />} />
          <Route path="map" element={<LoadMapView role="driver" />} />
          <Route path="chat" element={<ChatListView />} />
          <Route path="settings" element={<SettingsView role="driver" />} />
        </Routes>
      </main>
